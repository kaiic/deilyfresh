...
class OrderPlaceView(LoginRequiredMixin,View):
    ...
    def post(self, request):
        ...
        # 组织上下文
        sku_ids = ','.join(sku_ids) #[1,25,3]->1,25,3
        context = {
            'skus':skus,
...
class OrderCommintView1(View):
    ...
    def post(self,request):
        ...
        # todo:创建订单核心任务
        #组织参数
        # 订单id：202002090139+用户的id   -》年月日时分秒+用户ID
        ...
        try:
            ...
            for sku_id in sku_ids:
                ...
                try:
                    # sku = GoodsSKU.objects.get(id=sku_id)
                    sku = GoodsSKU.objects.select_for_update().get(id=sku_id)       #加锁select_for_update()，悲观锁，阻塞
                except:
                    #商品不存在
                    # 回滚事务保存点
...
class OrderCommintView(View):
    ...
    def post(self,request):
        ...
        try:
            ...
            cart_key = 'cart_%d'%user.id
            for sku_id in sku_ids:
                for i in range(3):  #循环三次，判断是否下单还是失败
                    #获取商品信息
                    try:
                        sku = GoodsSKU.objects.get(id=sku_id)
                        # sku = GoodsSKU.objects.select_for_update().get(id=sku_id)       #加锁select_for_update()，悲观锁，阻塞
                    ...
                    # todo:更新商品的库存和销量
                    orgin_stock = sku.stock     #之前库存
                    new_stock = orgin_stock - int(count)    #更新的库存
                    new_sales = sku.sales + int(count)      #更新的销量

                    # update df_goods_sku set stock=new_stock,sales=new_sales where id=sku_id and stock=orgin_stock
                    # 返回受影响的行数
                    # 把更新放到添加记录前面，没更新成功就不往表里加信息，否则就会出现向表里多次加同一信息的情况
